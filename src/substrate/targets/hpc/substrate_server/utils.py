...
def read_and_forward_pty_output(server, ns):
    ...
    while True:
        socketio.sleep(0.01)
        if app.config[f'{ns}_fd']:    
            timeout_sec = 0
            (data_ready, _, _) = select.select([app.config[f'{ns}_fd']], [], [], timeout_sec)
            if data_ready:
                output = os.read(app.config[f'{ns}_fd'], max_read_bytes).decode()
                socketio.emit("pty-output", {"output": output}, namespace=ns)
